...
- [x] 调试模式：后台管理链接始终显示
- [x] 主页添加后台管理访问按钮
- [ ] TODO：生产环境恢复登录验证（Admin.tsx + MainLayout.tsx）
- [ ] TODO：生产环境移除或隐藏主页后台管理按钮
...
- [x] 移除剪辑合成左侧面板（无实际功能，浪费空间）
- [x] ToolLayout组件支持可选左侧面板
- [x] 布局自动调整，内容区域更宽敞

## v2.4.2 UI统一优化（2026-01-19）
- [x] 统一分镜生成器UI样式与剧本生成器一致
- [x] 统一镜头卡UI样式与剧本生成器一致
- [x] 统一剪辑合成UI样式与剧本生成器一致
- [x] 间距优化：表单项、卡片、列表项间距统一为24px
- [x] 标签样式优化：字体加粗、颜色强调、层次清晰
- [x] 输入框样式优化：白色背景、边框加粗、交互效果
- [x] 按钮样式优化：渐变背景、高度增加、阴影效果
- [x] 卡片样式优化：内边距增加、背景半透明、边框加粗
- [x] 右侧面板优化：统计信息卡片、数值高亮显示
- [x] 文本域行数优化：增加显示行数
- [x] 创建UI统一优化文档（UI_UNIFICATION.md）
