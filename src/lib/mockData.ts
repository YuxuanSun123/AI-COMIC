...
export function initMockData(): void {
  // 动态导入避免循环依赖
  import('./localTableClient').then(({ localTableClient }) => {
    // 初始化各表数据
    localTableClient.initTable('users', mockUsers);
    localTableClient.initTable('news', mockNews);
    localTableClient.initTable('links', mockLinks);
    localTableClient.initTable('works', mockWorks);
  });
}
